...
import { GestureSequenceEngine } from "@/lib/gestureSequenceEngine";
...
export default function SignsPage() {
  ...
  const [recordWord, setRecordWord] = useState("");
  const [recordCategory, setRecordCategory] = useState("Пользовательские");
  const [recordLanguage, setRecordLanguage] = useState("ru");
  ...
  const engineRef = useRef<GestureSequenceEngine | null>(null);

  useEffect(() => {
    engineRef.current = new GestureSequenceEngine();
  }, []);
  ...
  const gesturesLibraryRef = useRef<any[]>([]);
  useEffect(() => {
    gesturesLibraryRef.current = gesturesLibrary;
  }, [gesturesLibrary]);

  const handleSaveGesture = async () => {
    if (!engineRef.current || !recordWord) return;
    setIsSaving(true);
    const pattern = engineRef.current.getCurrentBuffer();
    
    if (pattern.length < 5) {
      alert("Слишком короткое движение. Попробуйте еще раз.");
      setIsSaving(false);
      return;
    }

    const res = await saveGesturePattern(recordWord.toLowerCase(), recordCategory, pattern);
    if (res.error) {
      alert("Ошибка сохранения: " + res.error);
    } else {
      alert(`Жест "${recordWord}" успешно сохранен!`);
      // Refresh library
      const libRes = await getGesturesLibrary();
      if (libRes.data) setGesturesLibrary(libRes.data);
      setRecordWord("");
  ...
  const onResults = (results: any) => {
    ...
    if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
      ...
      if (engineRef.current) {
        engineRef.current.addFrame(landmarks);
        
        // Update Motion Energy
        const energy = engineRef.current.getMotionEnergy();
        setMotionEnergy(energy);

        // 1. Dynamic Library-based recognition
        // We only match if we are NOT actively recording
        if (!isRecording) {
          const matchResult = engineRef.current.match(gesturesLibraryRef.current);
          if (matchResult.word) {
            setRecognizedWord(matchResult.word);
            // Hide the recognized word after 3 seconds
            setTimeout(() => setRecognizedWord(null), 3000);
          }
        }
      }
    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto pb-20 relative">
      ...
      {activeTab === "camera" && (
        <div className="flex flex-col items-center space-y-6 animate-in fade-in duration-300">
            ...
            {isCameraActive && (
              <>
                <div className="absolute top-4 left-4 flex gap-2 z-20">
                  ...
                  {isRecordingMode && (
                    <div className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 backdrop-blur-md border ${isRecording ? 'bg-red-500/20 border-red-500/40 text-red-400' : 'bg-blue-500/20 border-blue-500/40 text-blue-400'}`}>
                      {isRecording ? (
                        <>
                          <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
                          ЗАПИСЬ ({(engineRef.current?.getCurrentBuffer().length || 0)} кадров)
                        </>
                      ) : 'РЕЖИМ ЗАПИСИ (Ожидание)'}
                    </div>
                  )}
                </div>
                
                {/* Motion Energy Bar (Vertical) */}
                <div className="absolute top-4 right-4 bottom-4 w-4 bg-white/10 rounded-full overflow-hidden border border-white/20 z-20 shadow-inner">
                  <div 
                    className="absolute bottom-0 w-full bg-gradient-to-t from-[var(--color-primary)] to-[#6CBEE8] transition-all duration-75"
                    style={{ height: `${Math.min(100, motionEnergy * 200)}%` }}
                  />
                </div>
              </>
            )}
          ...
          {/* Recording UI Panel */}
          {isRecordingMode && isCameraActive && (
            <div className="w-full max-w-2xl p-6 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] animate-in slide-in-from-top-4 duration-300">
              <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
                <Video className="text-[var(--color-primary)]" size={24} />
                Записать эталон
              </h3>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <input 
                  ...
                  value={recordWord}
                  onChange={e => setRecordWord(e.target.value)}
                  className="w-full p-3 rounded-xl border border-[var(--border-color)] bg-[var(--surface)] outline-none"
                />
                <select 
              ...
              <div className="flex gap-4">
                <button
                  ...
                  disabled={!recordWord}
                  className={`flex-1 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${!recordWord ? 'opacity-50 cursor-not-allowed bg-gray-500/20' : isRecording ? 'bg-red-500/20 text-red-500 border border-red-500/30' : 'bg-blue-500/20 text-blue-500 border border-blue-500/30 hover:bg-blue-500/30'}`}
                >
                ...
                <button
                  ...
                  disabled={isRecording || !recordWord || isSaving || (engineRef.current?.getCurrentBuffer().length || 0) === 0}
                  className={`flex-1 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${isRecording || !recordWord || isSaving || (engineRef.current?.getCurrentBuffer().length || 0) === 0 ? 'opacity-50 cursor-not-allowed bg-gray-500/20' : 'bg-[var(--color-primary)] text-white hover:shadow-lg'}`}
                >
                  {isSaving ? 'Сохранение...' : 'Сохранить жест'}
                </button>
              </div>
            </div>
          )}
          ...
          <div className="flex gap-4 w-full max-w-2xl">
            <button
              ...
              className={`flex-1 py-5 rounded-2xl font-bold text-lg transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3
                ${isCameraActive 
            ...
            {isCameraActive && (
              <button
                ...
                className={`w-16 flex items-center justify-center rounded-2xl border-2 transition-all ${isRecordingMode ? 'border-[var(--color-primary)] bg-[var(--color-primary)]/10 text-[var(--color-primary)]' : 'border-[var(--border-color)] bg-[var(--bg-card2)] hover:bg-[var(--surface)]'}`}
                title="Режим записи эталонов"
              >
                <Video size={24} />
              </button>
            )}
